...
Future<SubmitKYCResponse> updateKYC({
  ...
  var request = http.MultipartRequest(
    'POST',
    Uri.parse('$baseUrlCustomer/${ApiEndpoint.updateKYC}'),
  );
  var body = {
  ...
  if (selfieImage?.mimeType == "http") {
    request.fields["photo_proof.file[0][file_id]"] =
        oldKYCData?.photoProof?.files?.first.id ?? "";
  } else {
      ...
      if (identityDoc.localFiles != null) {
        for (int j = 0; j < identityDoc.localFiles!.length; j++) {
          ...
          if (doc?.mimeType == "http") {
            ...
            request.fields["add_proof[$i].files[$j][file_id]"] =
                oldDoc.id ?? "";
          } else {
    ...
    for (int i = 0; i < identityProofList.length; i++) {
      ...
      if (identityDoc.localFiles != null) {
        for (int j = 0; j < identityDoc.localFiles!.length; j++) {
          ...
          if (doc?.mimeType == "http") {
            var oldDoc = identityDoc.files![j];
            request.fields["identity_proof[$i].files[$j][file_id]"] =
                oldDoc.id ?? "";
          } else {
            var multiPartFile = doc == null
                ? null
